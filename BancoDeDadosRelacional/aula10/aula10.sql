...
GROUP BY nome_departamento, nome_cidade;
...
CREATE VIEW v_opsalario
AS SELECT SUM(salario) as soma, AVG(salario) as media
FROM tbl_empregados;
...
DROP VIEW v_departemp;
CREATE VIEW v_departemp AS
SELECT d.nome AS "Departamento", COUNT (e.cod_empregado) AS "Quantidade"
FROM tbl_departamentos d
JOIN tbl_empregados e 
