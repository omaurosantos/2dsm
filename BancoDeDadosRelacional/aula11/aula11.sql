...
CREATE OR REPLACE PROCEDURE proc_upd_nome_depart(
    ...
    IN novo_nome_departamento TEXT
)
LANGUAGE plpgsql
AS $$
BEGIN
    UPDATE tbl_departamentos
    SET nome = novo_nome_departamento
...
BEGIN
    ...
    VALUES (nome_projeto, cod_departamento);
END;
$$;
...
AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tbl_projetos_arquivados')
    THEN
