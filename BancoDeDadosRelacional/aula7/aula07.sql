...
select upper(f.nome), lower(p.nome) from tbl_fornecedor as f join tbl_estoque as e on f.cod_fornecedor = e.cod_fornecedor join tbl_peca as p on p.cod_peca = e.cod_peca order by f.nome;
...
select p.nome, p.cor from tbl_peca as p join tbl_estoque as e on p.cod_peca = e.cod_peca join tbl_fornecedor as f 
on f.cod_fornecedor = e.cod_fornecedor where f.cod_fornecedor = 3 order by p.nome;

--4 Listar o nome e a cidade dos fornecedores com mais de 10 peças. Contar só as peças de código 1. 
select f.nome, f.cidade from tbl_fornecedor as f join tbl_estoque as e on f.cod_fornecedor = e.cod_fornecedor join tbl_peca as p on e.cod_peca = p.cod_peca where e.quantidade > 10 AND p.cod_peca = 1;

--5 Listar a quantidade total de peças com código 1, fornecidas pelos fornecedores.
select sum(e.quantidade) from tbl_estoque as e join tbl_peca as p on e.cod_peca = p.cod_peca where p.cod_peca = 1;
