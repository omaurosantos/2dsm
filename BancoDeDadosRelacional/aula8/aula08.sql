...
SELECT nome FROM tbl_peca WHERE preco > 10 AND cod_peca IN (SELECT cod_peca FROM tbl_estoque WHERE quantidade > 0);
...
SELECT * FROM tbl_peca WHERE nome != 'PLACA' order by nome;

--6 Listar o nome e a cor das peças do fornecedor C, ordenado pelo nome da peça
SELECT nome, cor FROM tbl_peca WHERE cod_peca IN (SELECT cod_peca FROM tbl_estoque WHERE cod_fornecedor = (SELECT cod_fornecedor FROM tbl_fornecedor WHERE nome = 'C')) order by nome;

--7 Listar o nome e a cor de todas as pecas de Londres
SELECT nome, cor FROM tbl_peca WHERE cidade IN (SELECT cidade FROM tbl_fornecedor WHERE cidade = 'LONDRES');

--8 Encontre o nome das peças que estão disponíveis em estoque em Londres e não estão disponíveis em estoque em Paris:
SELECT nome, cidade FROM tbl_peca WHERE cod_peca IN (SELECT cod_peca FROM tbl_estoque WHERE cod_peca IN (SELECT cod_peca FROM tbl_peca WHERE cidade = 'LONDRES') AND cod_peca NOT IN (SELECT cod_peca FROM tbl_estoque WHERE cod_peca IN (SELECT cod_peca FROM tbl_peca WHERE cidade = 'PARIS')));
